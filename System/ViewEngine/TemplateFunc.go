...
func RenderView(strViewName string, viewData map[string]interface{}) HTML {
	var strController, strTheme string
	if strViewName == "" {
	...
	err := viewEngine.RenderView(strController, strViewName, strTheme, viewData, writer)
	if err != nil {
		Log.AppLog.AddErrMsg("RenderView出错 controller:" + strController + ",viewName:" + strViewName + "\r\n" + err.Error())
	}
