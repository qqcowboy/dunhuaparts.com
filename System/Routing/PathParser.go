...
import (
	...
	"github.com/qqcowboy/dunhuaparts.com/System/Routing/PathSegmentType"
)
...
func (this *PathParser) Match(requestPath string, defaults map[string]interface{}) map[string]interface{} {
	...
	ret["views"] = ""
	for pathSegCount > routSegCount {
		views := fmt.Sprintf("%v", ret["views"])
		if len(views) > 0 {
			ret["views"] = fmt.Sprintf("%s/%s", strings.Title(arrPathSegs[pathSegCount-1]), ret["views"])
			fmt.Println(ret["views"], arrPathSegs[pathSegCount-1])
		} else {
			ret["views"] = strings.Title(arrPathSegs[pathSegCount-1])
		}
		arrPathSegs = arrPathSegs[:(pathSegCount - 1)]
		pathSegCount--
	}
	//对URL的段数进行比较
